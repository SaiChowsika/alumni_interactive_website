...
const TermsAndConditions = () => {
  ...
  const fetchSubmissions = async () => {
    try {
      ...
      const response = await axios.get(`${API_BASE_URL}/api/placements/user/${user.id}`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post(`${API_BASE_URL}/api/placements`, submissionData, {
        headers: { Authorization: `Bearer ${token}` }
      });
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="flex space-x-4">
                    <label className="inline-flex items-center">
                      <input
                        type="radio"
                        value="placement"
                        checked={formData.jobType === 'placement'}
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    {formData.jobType === 'placement' ? 'Package (LPA)' : 'Stipend (per month)'}
                  </label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    {formData.jobType === 'placement' ? 'Joining Date' : 'Internship Start Date'}
                  </label>
                  <input
                    type="date"
            ...
            <div key={index} className="bg-gray-50 rounded-lg p-4 flex flex-col md:flex-row justify-between items-start md:items-center space-y-2 md:space-y-0">
              <div>
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-sm text-gray-500">•</span>
                  <p className="text-sm text-gray-600">{submission.jobType}</p>
                </div>
